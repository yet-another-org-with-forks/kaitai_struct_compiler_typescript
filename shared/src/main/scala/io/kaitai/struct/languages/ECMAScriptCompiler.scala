...
/**
 * Base compiler for JavaScript and TypeScript.
 * Since TypeScript is a subset of JavaScript, most of the code is shared.
 */
abstract class ECMAScriptCompiler(typeProvider: ClassTypeProvider, config: RuntimeConfig)
  ...
  protected def static: ECMAScriptCompilerStatic
  ...
  override val translator: ECMAScriptTranslator
  ...
  override def externalTypeDeclaration(extType: ExternalType): Unit = {
    ...
    static.importExternalTypeDeclaration(importList, className)
  }
  ...
  override def attrProcess(proc: ProcessExpr, varSrc: Identifier, rep: RepeatSpec): String = {
    ...
    proc match {
      ...
      case ProcessCustom(name, args) =>
        static.importProcessor(importList, name, config)
  ...
  override def parseExpr(dataType: DataType, io: String, defEndian: Option[FixedEndian]): String = {
    dataType match {
      ...
      case t: UserType =>
        ...
        s"new ${static.userTypeName(t)}($io, $parent, $root$addEndian$addParams)"
    }
  }
  ...
  override def attrParentParamCheck(actualParentExpr: Ast.expr, ut: UserType, shouldDependOnIo: Option[Boolean], msg: String): Unit = {
    /** @note Must be kept in sync with [[ECMAScriptCompiler.parseExpr]] */
  ...
  private def tsType(attrType: DataType, isNullable: Boolean = false): String =
    static.kaitaiType2NativeType(attrType, isNullable, config)
  ...
  override def attrValidateInEnum(
    ...
    val enumRef = static.enumTypeName(et)
...
trait ECMAScriptCompilerStatic extends LanguageCompilerStatic
  with UpperCamelCaseClasses
  ...
  def importExternalTypeDeclaration(importList: ImportList, name: String): Unit

  def importProcessor(importList: ImportList, name: List[String], config: RuntimeConfig): Unit
