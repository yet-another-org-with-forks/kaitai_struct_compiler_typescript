...
import io.kaitai.struct.languages.ECMAScriptCompilerStatic
import io.kaitai.struct.{ImportList, RuntimeConfig, Utils}

abstract class ECMAScriptTranslator(provider: TypeProvider, importList: ImportList, config: RuntimeConfig) extends BaseTranslator(provider) {
  protected def compilerStatic: ECMAScriptCompilerStatic
